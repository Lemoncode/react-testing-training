describe('Login specs', () => {
  ...
  it('should name input has the focus when it clicks on it', () => {
    ...
    // Act
    cy.visit('/');
    cy.findAllByLabelText('Name').click();

    // Assert
    cy.findAllByLabelText('Name').should('have.focus');
  });
  ...
  it('should show an alert with a message when type invalid credentials', () => {
    ...
    cy.findAllByLabelText('Name').as('userInput');
    cy.findByLabelText('Password').as('passInput');
    ...
    cy.get('@passInput').should('have.value', password);
